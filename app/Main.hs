...
main = do
    ...
    scottyOpts (Options 0 (setPort port $ setHost "0.0.0.0" defaultSettings)) $ do
        ...
        post "/login" $ do
            ...
            let n = trimSpaces (TL.unpack fullName)
        ...
        get "/dashboard" $ do
            displayName <- requireLogin sessionsRef
            csvData <- liftIO $ BL.readFile "data/data1.csv"
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                Left err -> html (TL.pack err)
                Right (_, records) -> html (searchPage (V.toList records) displayName)
        ...
        get "/dataset" $ do
            ...
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                Left err -> html (TL.pack err)
                Right (_, records) -> html (datasetPage displayName (V.toList records))

        get "/methodology" $ do
            displayName <- requireLogin sessionsRef
            html (methodologyPage displayName)

        get "/compare-species" $ do
            displayName <- requireLogin sessionsRef
            csvData <- liftIO $ BL.readFile "data/data1.csv"
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                Left err -> html (TL.pack err)
                Right (_, records) -> html (compareSpeciesBody displayName (V.toList records))

        post "/compare-species" $ do
            displayName <- requireLogin sessionsRef
            sp1 <- formParam "species1"
            sp2 <- formParam "species2"
            ar  <- formParam "area"
            csvData <- liftIO $ BL.readFile "data/data1.csv"
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                Left err -> html (TL.pack err)
                Right (_, records) -> do
                    let rs = V.toList records
                    let r1 = sortOn year (filterRecords (TL.unpack sp1) (TL.unpack ar) rs)
                    let r2 = sortOn year (filterRecords (TL.unpack sp2) (TL.unpack ar) rs)
                    html (compareSpeciesResultPage displayName (TL.unpack sp1) (TL.unpack sp2) (TL.unpack ar) r1 r2)

        get "/compare-areas" $ do
            displayName <- requireLogin sessionsRef
            csvData <- liftIO $ BL.readFile "data/data1.csv"
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                Left err -> html (TL.pack err)
                Right (_, records) -> html (compareAreasBody displayName (V.toList records))

        post "/compare-areas" $ do
            displayName <- requireLogin sessionsRef
            sp  <- formParam "species"
            ar1 <- formParam "area1"
            ar2 <- formParam "area2"
            csvData <- liftIO $ BL.readFile "data/data1.csv"
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                ...
                Right (_, records) -> do
                    let rs = V.toList records
                    let r1 = sortOn year (filterRecords (TL.unpack sp) (TL.unpack ar1) rs)
                    let r2 = sortOn year (filterRecords (TL.unpack sp) (TL.unpack ar2) rs)
                    html (compareAreasResultPage displayName (TL.unpack sp) (TL.unpack ar1) (TL.unpack ar2) r1 r2)
        ...
        get "/risk-ranking" $ do
            ...
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                Left err -> html (TL.pack err)
                Right (_, records) -> html (riskRankingPage displayName (V.toList records))
        ...
        get "/calculator" $ do
            ...
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                Left err -> html (TL.pack err)
                Right (_, records) -> html (calculatorPage displayName (V.toList records) Nothing)

        post "/calculator" $ do
            displayName <- requireLogin sessionsRef
            sp <- formParam "species"
            ar <- formParam "area"
            currentVal <- formParam "current"
            let currentTons = read (TL.unpack currentVal) :: Double
            csvData <- liftIO $ BL.readFile "data/data1.csv"
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                Left err -> html (TL.pack err)
                Right (_, records) -> do
                    let allRecords = V.toList records
                    let rs = sortOn year (filterRecords (TL.unpack sp) (TL.unpack ar) allRecords)
                    let catches = getCatchValues rs
                    let prevAvg = meanList catches
                    let lastKnown = if null catches then 0 else last catches
                    let pctFromLast = safePercentChange lastKnown currentTons
                    html (calculatorPage displayName allRecords (Just (TL.unpack sp, TL.unpack ar, currentTons, prevAvg, lastKnown, pctFromLast)))
            ...
            case (decodeByName csvData :: Either String (Header, V.Vector FishRecord)) of
                ...
                Right (_, records) -> do
                    ...
                    then html $
                        ...
                        let diffs = yearlyDiff catches
                        let declineCount = countDeclines diffs
                        let totalChanges = length diffs
                        let status = sustainabilityStatus declineCount totalChanges
                        html (resultsPage sp ar yearsList catches declineCount status displayName)